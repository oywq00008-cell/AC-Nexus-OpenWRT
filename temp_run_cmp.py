...
for rel in common:
    ...
    if h1 != h2:
        diff_files.append((rel, h1[:8], h2[:8]))

if diff_files:
    print(f"\nContent differs ({len(diff_files)}):")
    for f, h1, h2 in diff_files:
        print(f"  ! {f}  [{h1} vs {h2}]")
else:
    print("\nAll common files have IDENTICAL content (MD5 match).")

print(f"\n=== VERDICT ===")
if not only_ipk and not only_bc and not diff_files:
    print("✅ ipk-build/data/ 和 broadlinkac/files/ 完全重复（结构+内容均一致）")
elif not only_ipk and not only_bc and diff_files:
    print("⚠️ 目录结构相同，但部分文件内容有差异")
else:
    print("❌ 两个目录不完全相同（结构或内容有差异）")
'''
