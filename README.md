...
**AC-Nexus-OpenWRT** 是一个跑在 OpenWRT 路由器上的**全自动空调控制插件**。把博联 RM 红外遥控器（或米家 MIoT 红外遥控器）插上路由器、装上本插件后，路由器就能 7×24 小时替你管空调——不用开电脑、不用守着。
...
> 一句话：**把红外遥控器插上路由器 → 装上本插件 → 以后空调全自动（按温度、按时间、台风来了自动关）。**
