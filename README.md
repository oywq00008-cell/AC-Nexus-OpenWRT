...
**AC-Nexus-OpenWRT** 是一个跑在 OpenWRT 路由器上的**全自动空调控制插件**。把博联 RM 红外遥控器（或米家 MIoT 红外遥控器）接上电源、连入局域网，装上本插件后，路由器就能 7×24 小时替你管空调——不用开电脑、不用守着。

它能帮你做这些事：

- 🏠 **7×24 小时无人值守** — 路由器常开即可，开机自启、挂了自动重启，装好就忘
- 🌡️ **按天气自动调温** — 自动拉取室外温度，热了开、冷了关、适时调温
- ⏰ **定时开关机** — 工作日 / 周末分别排程，到点自动执行
- 🌀 **台风自动保护** — 风暴临近按三级判定自动关机，保护外机不被大风吹坏
- 🔧 **Web 面板管理** — 浏览器打开路由器即可遥控、设规则、看日志

> 一句话：**红外遥控器接上电源、连入局域网 → 装上本插件 → 以后空调全自动（按温度、按时间、台风来了自动关）。**
